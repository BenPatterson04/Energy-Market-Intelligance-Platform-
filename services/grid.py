...
def get_full_generation_mix():
    """
    Full UK generation mix (all fuel types, not just the top 3 shown on
    the dashboard card) - used by the dedicated Grid page for a proper
    breakdown chart.
    """
    ...
    try:
        ...
        mix = response.json()["data"]["generationmix"]
        mix = sorted(mix, key=lambda x: x["perc"], reverse=True)

        return [
            {"fuel": item["fuel"].replace("-", " ").title(), "percent": item["perc"]}
            for item in mix
        ]

    except Exception as e:
        print("Grid generation mix error:", e)
        return []
